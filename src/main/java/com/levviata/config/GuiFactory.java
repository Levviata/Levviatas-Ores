    ...
    public static class LOConfigGui extends GuiConfig
    {
        public LOConfigGui(GuiScreen parentScreen)
        {
            ...
            super(parentScreen, getConfigElements(), Reference.MODID,
                    false, false, I18n.format("gui.lo_configuration.mainTitle"));
        }
        ...
        private static List<IConfigElement> getConfigElements()
        {
            ...
            list.add(new DummyCategoryElement("blocksCfg", "gui.lo_configuration.ctgy.blocks", CategoryEntryBlocks.class));
            list.add(new DummyCategoryElement("itemsCfg", "gui.lo_configuration.ctgy.items", CategoryEntryItems.class));
        ...
        public static class CategoryEntryBlocks extends CategoryEntry {
            public CategoryEntryBlocks(GuiConfig owningScreen, GuiConfigEntries owningEntryList, IConfigElement prop) {
                super(owningScreen, owningEntryList, prop);
            }
            ...
            protected GuiScreen buildChildScreen()
            {
                ...
                Configuration configuration = Config.getConfig();
                ConfigElement cat_general = new ConfigElement(configuration.getCategory(Config.CATEGORY_NAME_BLOCKS));
                List<IConfigElement> propertiesOnThisScreen = cat_general.getChildElements();
                String windowTitle = configuration.toString();

                return new GuiConfig(this.owningScreen, propertiesOnThisScreen,
                        this.owningScreen.modID,
                        Config.CATEGORY_NAME_BLOCKS,
                        this.configElement.requiresWorldRestart() || this.owningScreen.allRequireWorldRestart,
                        this.configElement.requiresMcRestart() || this.owningScreen.allRequireMcRestart,
                        windowTitle);
                //this is a complicated object that specifies the category's gui screen, to better understand
